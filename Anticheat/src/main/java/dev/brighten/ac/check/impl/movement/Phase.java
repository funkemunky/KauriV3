...
public class Phase extends Check {
    ...
    private int ticks;
    private MovementHandler.TeleportAction teleported = null;
    private Location teleportLoc = null;

    @Bind
    WAction<ConfirmedPositionEvent> positionOut = packet -> {
        teleported = packet.action();
    };
    ...
    WCancellable<WrapperPlayClientPlayerFlying> packet = (packet) -> {
        ...
        if(teleported != null) {
            if (teleported.position.distance(player.getMovement().getTo().getLoc()) < 0.001) {
